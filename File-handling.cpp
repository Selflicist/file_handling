...
int main()
{
   string f1, f2;
	...
	cin>>f2;
	ofstream file1(f1);
	file1<<"File handling in C++ is a mechanism to store the output of a program in a file";
	file1.close();
	ofstream file2(f2);
	file2<<"Files help store these data permanently on a storage device easily";
	file2.close();
	ifstream fin1, fin2;
	fin1.open(f1);
